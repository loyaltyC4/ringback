...
// ("Grain Gradient"). Zero dependencies: one WebGL canvas that fills its
...
/** hex -> normalised rgb triplet, padded to the 8 slots the shader declares */
export function palette(hexes: string[]): [number, number, number][] {
...
const DEFAULTS: ShaderUniforms = {
  colors: [[0.0392156862745098,0.0392156862745098,0.0392156862745098],[0.24705882352941178,0.23529411764705882,0.7333333333333333],[0.8588235294117647,0.29411764705882354,0.5607843137254902],[1,0.8392156862745098,0.4196078431372549],[1,0.8392156862745098,0.4196078431372549],[1,0.8392156862745098,0.4196078431372549],[1,0.8392156862745098,0.4196078431372549],[1,0.8392156862745098,0.4196078431372549]] as [number, number, number][],
  colorCount: 4,
  scale: 1.260,
  intensity: 0.350,
  paramA: 0.280,
...
export function ShaderBackground({
  ...
  useEffect(() => {
    const UNIFORMS: ShaderUniforms = { ...DEFAULTS, ...uniforms }
    // A shader that animates forever is wasted motion for anyone who asked
    // the OS to stop it; freeze on the first frame instead.
    ...
    function render(now: number) {
      ...
      // re-assert for the type checker: narrowing from the guards above is not
      // carried into a hoisted function declaration.
      if (!canvas || !gl) return
